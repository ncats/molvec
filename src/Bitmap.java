...
public class Bitmap implements Serializable {
	...
	public Point add (ChainCode code) { 
	    Point pt = coords.getLast();
	    //logger.info(pt.toString()+" "+code);
	    Point newPt = new Point (pt.x+code.dx(), pt.y+code.dy());
	    if (!contains (newPt)) {
		coords.add(newPt);
		codes.add(code);
	    }
	...
	public Point getStartPt () { return start; }
	public int length () { return codes.size();}
	public Point[] getCoords () {
	    return coords.toArray(new Point[0]);
	}
	public ChainCode getCode (Point pt) {
	    return getCode (pt.x, pt.y);
	}
	...
	public Point[] dominantPoints (double threshold) {
	    ...
	    boolean open = cc[i].x != cc[0].x || cc[i].y != cc[0].y;
	    if (open) {
	    ...
	    if (DEBUG) {
		System.out.println("## "+breaks.size()+" dominant points!");
		for (i = 0; i < breaks.size(); ++i) {
		    AEV aev = breaks.get(i);
		    Point pt = cc[aev.k];
		    System.out.println("** dominant point at "+pt+"; aev = "+aev.dist);
		    if (i+1 < breaks.size()) {
	    ...
	    Point[] pts = new Point[breaks.size()];
	    for (i = 0; i < breaks.size(); ++i) {
		AEV aev = breaks.get(i);
		pts[i] = cc[aev.k];
	    }
	...
	void calcAEV (List<AEV> breaks, Point[] cc, 
		...
		if (DEBUG) {
		    Point pt = cc[aev.k];
		    System.out.println("** break point at "+pt+"; aev = "+aev.dist);
		    if (i+1 < breaks.size()) {
			for (int k = aev.k; k <= breaks.get(i+1).k; ++k) {
			    System.out.println("  ++ "+cc[k]);
			}
	...
	AEV calcAEV (int i, List<AEV> breaks, Point[] cc, boolean closed) {
		...
		if (i == 0) {
		    Point pi = cc[breaks.get(size-1).k];
		    Point pj = cc[breaks.get(i+1).k];
		    aev.dist = sqDist (pt, pi, pj);
		}
		else { // i+1 == breaks.size()
		    Point pi = cc[breaks.get(i-1).k];
		    Point pj = cc[breaks.get(0).k];
		    aev.dist = sqDist (pt, pi, pj);
		}
	    }
	...
	static double sqDist (Point pk, Point pi, Point pj) {
	    double a = (pk.x-pi.x)*(pj.y-pi.y) - (pk.y-pi.y)*(pj.x-pi.x);
	    double b = (pi.x-pj.x);
	    double c = (pi.y-pj.y);
	    return a*a/(b*b + c*c);
	}
    ...
    public Object clone () { return new Bitmap (this); }
    ...
    /* Thinning algorithm based on Zhang and Suen.  The following description
       is based on 
	...
	while (true) {
		...
		for (int x = 0; x < width; ++x) {
			...
			&& ((x == 0 || y == 0	  /* boundary */
			    ...
			    || !thin.get(x-1, y-1) || !thin.get(x+1, y-1)
			    || !thin.get(x-1, y+1) || !thin.get(x+1, y+1))) {
	...
	// equivalence class
	short[] eqvtab = new short[5000]; // some initial default
	short []L = new short[4];
    ...
    public List<Point> dominantPoints () {
	return dominantPoints (5, DEFAULT_AEV_THRESHOLD);
    }

    public List<Point> dominantPoints (int minsize, double threshold) {
	List<Point> dps = new ArrayList<Point>();
	for (ChainCodeSequence ccs : chainCodes (minsize)) {
	    for (Point pt : ccs.dominantPoints(threshold)) {
		dps.add(pt);
	    }
	}
	return dps;
    ...
    public List<GeneralPath> segments () {
	return segments (5, DEFAULT_AEV_THRESHOLD);
    }

    public List<GeneralPath> segments (int minsize, double threshold) {
	List<GeneralPath> segs = new ArrayList<GeneralPath>();
	for (ChainCodeSequence ccs : chainCodes (minsize)) {
	    GeneralPath gp = null;
	    for (Point pt : ccs.dominantPoints(threshold)) {
		if (gp == null) {
	...
	if (Nb.isEmpty()) {
	    ...
	    Point pt = seq.add(Nb.iterator().next());
	    if (pt != null) {
		chainCode2 (seq, visited, bitmap, pt.x, pt.y);
	    }
	}
	else {
	    // multiple choice; pick best one based on the following
	    ...
	    if (best != null) {
		Point pt = seq.add(best);
		if (pt != null) {
		    chainCode2 (seq, visited, bitmap, pt.x, pt.y);
		    bitmap.set(pt.x, pt.y, true);
		}
	    }
	...
	*/
	...
	List<Point> dps = tif.dominantPoints();
